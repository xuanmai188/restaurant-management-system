...
if (isset($_GET['unlock'])) {
    $id = (int)$_GET['unlock'];
    $conn->query("UPDATE users SET status='hoat_dong' WHERE user_id=$id AND role_id != 1");
    $msg = 'Đã mở khóa tài khoản.'; $msgType = 'alert-success';
    $key = $_GET['key'] ?? '';
    header('Location: admin.php?page=employees' . ($key ? '&key='.$key : '')); exit;
}
...
<div class="card panel">
    <div class="table-wrap">
        <table class="table">
            <thead><tr><th>Họ tên</th><th>Username</th><th>SĐT</th><th>Email</th><th>Giới tính</th><th>Vai trò</th><th>Trạng thái</th><th>Thao tác</th></tr></thead>
            <tbody>
            <?php if ($employees && $employees->num_rows > 0): ?>
                <?php while ($row = $employees->fetch_assoc()): 
                    $isLocked = ($row['status'] === 'khoa');
                ?>
                <tr>
                    <td style="<?= $isLocked ? 'color:#9ca3af;' : '' ?>"><?= e($row['full_name']) ?></td>
                    <td style="<?= $isLocked ? 'color:#9ca3af;' : '' ?>"><?= e($row['username']) ?></td>
                    <td style="<?= $isLocked ? 'color:#9ca3af;' : '' ?>"><?= e($row['phone']) ?></td>
                    <td style="<?= $isLocked ? 'color:#9ca3af;' : '' ?>"><?= e($row['email']) ?></td>
                    <td style="<?= $isLocked ? 'color:#9ca3af;' : '' ?>"><?= e($row['gioi_tinh'] ?: '-') ?></td>
                    <td>
                        <select class="select" style="padding:6px 10px;font-size:13px;width:auto;<?= $isLocked ? 'pointer-events:none;opacity:0.5;' : '' ?>" onchange="if(confirm('Thay đổi vai trò thành ' + this.options[this.selectedIndex].text + '?')) location.href='admin.php?page=employees&change_role=<?= $row['user_id'] ?>&new_role=' + this.value; else this.value='<?= $row['role_id'] ?>';">
                    ...
                    <td>
                        <div style="display:flex; gap:8px; align-items:center; flex-wrap:wrap;">
                            <?php if ($isLocked): ?>
                                <a href="admin.php?page=employees&unlock=<?= $row['user_id'] ?>" 
                                   class="btn" 
                                   style="background:linear-gradient(135deg,#16a34a,#15803d);color:#fff;font-size:13px;padding:8px 16px;"
                                   onclick="return confirm('Mở khóa tài khoản \"<?= e($row['full_name']) ?>\"?')">Mở khóa</a>
                                <a href="admin.php?page=employees&delete=<?= $row['user_id'] ?>" 
                                   class="btn"
                                   style="background:linear-gradient(135deg,#ef4444,#dc2626);color:#fff;font-size:13px;padding:8px 16px;"
                                   onclick="return confirm('Xóa vĩnh viễn nhân viên này?')">Xóa</a>
                            <?php else: ?>
                                <button class="btn btn-primary" style="font-size:13px;padding:8px 16px;" onclick="editEmployee(<?= $row['user_id'] ?>)">Sửa</button>
                                <a href="admin.php?page=employees&delete=<?= $row['user_id'] ?>" 
                                   class="btn btn-danger"
                                   style="font-size:13px;padding:8px 16px;"
                                   onclick="return confirm('Xóa nhân viên này vĩnh viễn?')">Xóa</a>
                            <?php endif; ?>
                        </div>
                    </td>
                </tr>
                <?php endwhile; ?>
            <?php else: ?>
                <tr><td colspan="8"><div class="empty-state">Không có nhân viên nào.</div></td></tr>
            <?php endif; ?>
            </tbody>
        </table>
    </div>
