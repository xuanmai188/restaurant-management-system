...
// ── AUTO REFRESH: Tự động cập nhật danh sách đơn chờ thanh toán ───────────────
let _isProcessing = false;

async function refreshCashierData() {
    if (_isProcessing || document.hidden) return;
    ...
    try {
        ...
        // Cập nhật danh sách đơn nếu có thay đổi
...
function updateStats(stats) {
    // Cập nhật số đơn chờ
    const pendingEl = document.querySelector('[style*="border-left:4px solid #d97706"] div[style*="font-size:28px"]');
    if (pendingEl && stats.total_pending !== undefined) {
        pendingEl.textContent = stats.total_pending;
        pendingEl.style.color = stats.total_pending > 0 ? '#d97706' : '#9ca3af';
    }
}

function updateOrdersList(orders) {
    // Đơn giản: nếu số lượng thay đổi, reload trang
    const currentCount = document.querySelectorAll('[data-order-id]').length;
    if (currentCount !== orders.length && orders.length > currentCount) {
        // Có đơn mới - reload để hiển thị
        location.reload();
    }
}

// Auto refresh mỗi 15 giây
setInterval(refreshCashierData, 15000);

// Đánh dấu đang xử lý khi submit form
