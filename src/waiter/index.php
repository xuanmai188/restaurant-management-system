...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_order'])) {
    ...
    if (!$tableExists) {
        ...
        $duration = reservation_duration_minutes($guest_count);
        ...
        if ($reservationConflict) {
            ...
            $msg = 'Bàn này đã được đặt trước từ ' . date('H:i d/m/Y', strtotime($conflictStart)) . ' đến ' . date('H:i d/m/Y', strtotime($conflictEnd)) . '. Bạn không thể tạo đơn hàng vào khung giờ này.';
            $msgType = 'alert-error';
        } else {
