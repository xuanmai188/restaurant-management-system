            ...
            <table style="width:100%; border-collapse:collapse;">
                ...
                <tbody id="logsTableBody">
                    ...
                    <tr data-action-type="<?= $action ?>" style="border-bottom:1px solid #f3f4f6;">
                        ...
                        <td style="padding:14px 12px; max-width:300px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; color:#6b7280; font-size:13px;">
                            <?php if ($log['backup_file']): ?>
                                <div style="display:flex; align-items:center; gap:12px;">
                                    <span style="flex:1;"><?= e($log['details'] ?? '-') ?></span>
                                    <button onclick="downloadBackup('<?= $log['backup_file'] ?>')" style="padding:6px 10px; border:none; border-radius:6px; cursor:pointer; background:linear-gradient(135deg,#10b981,#059669); color:white; font-size:13px;">
                                        Tải xuống
                                    </button>
                                </div>
                            <?php else: ?>
